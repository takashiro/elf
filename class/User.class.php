...
class User extends DBObject{
	...
	public function insert($extra = ''){
		runhooks('user_register', array($this));
		parent::insert($extra);
	}
	...
	public static function Register($post){
		...
		if(!array_key_exists('account', $post) || !array_key_exists('password', $post)){
			return 0;
		}

		$account_length = strlen($post['account']);
		if($account_length < 4 || $account_length > 15){
			return self::INVALID_ACCOUNT;
		}

		if(!preg_match('/^[0-9a-z\x{4e00}-\x{9fa5}]+$/iu', $post['account'])){
			return self::INVALID_ACCOUNT;
		}

		$password_length = strlen($post['password']);
		...
		$user->pwmd5 = rmd5($post['password']);
		$user->regtime = TIMESTAMP;

		if(self::Exist($user->account, 'account')){
